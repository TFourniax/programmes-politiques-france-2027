...
export default function DataPage() {
  ...
  const topics = meta.topics.map(topic => {
    ...
    const direct = data.candidates.filter(row => ["documented","partial"].includes(row.coverage.level)).length;
  ...
  const watchStatus = health.status || "unknown";
  const watchFresh = health.last_collection_success_at || health.generated_at || null;
  ...
  return <main className="seoPage">
    ...
    <section className="seoHero"><span className="publicEyebrow">Audit public du corpus</span><h1>Couverture, fraîcheur et limites visibles</h1><p>Un corpus politique fiable doit permettre de voir ce qu’il sait autant que ce qu’il ne sait pas encore. Cette page expose la couverture actuelle, l’état de la veille et les règles qui empêchent de transformer une lacune documentaire en position politique.</p><div className="seoMetaRow"><span>instantané canonique {meta.snapshotDate}</span><span>veille : {watchStatus}</span>{watchFresh && <span>dernière collecte {watchFresh}</span>}</div></section>
    ...
    <section className="seoSection"><div className="seoSectionHeading"><div><span className="publicEyebrow">Couverture par enjeu</span><h2>Où le corpus est dense — et où il doit encore progresser</h2></div></div><div className="seoCardGrid">{topics.map(topic=><Link className="seoCard" href={`/themes/${topic.id}`} key={topic.id}><strong>{topic.label}</strong><p>{topic.direct} personnalité(s) avec élément direct · {topic.parties} parti(s) avec éléments indexés.</p><div className="seoMetaRow"><span>{pct(active.length ? topic.direct / active.length : 0)} des candidatures actives</span></div></Link>)}</div></section>

    <section className="seoSection"><div className="seoSectionHeading"><div><span className="publicEyebrow">Chaîne de fraîcheur</span><h2>Veille et promotion canonique</h2></div><p>La collecte peut tourner plus souvent que la publication du site. Les changements vérifiés sont regroupés avant une publication canonique afin de limiter les builds sans ralentir la veille.</p></div><div className="coverageStrip"><div><strong>{watchStatus}</strong><span>état de la veille</span></div><div><strong>{health.pending_work ?? 0}</strong><span>éléments en attente</span></div><div><strong>{health.actionable_pending_work ?? health.pending_work ?? 0}</strong><span>travail actionnable</span></div><div><strong>{health.persistent_official_source_failures ?? 0}</strong><span>échecs persistants de sources officielles</span></div></div>{Array.isArray(health.reasons) && health.reasons.length > 0 && <div className="seoMetaRow">{health.reasons.map(reason=><span key={reason}>{reason}</span>)}</div>}</section>

    <section className="seoSection"><div className="seoSectionHeading"><div><span className="publicEyebrow">Données ouvertes</span><h2>Réutiliser le corpus</h2></div><p>Le dépôt Markdown reste la source de vérité. Les interfaces machine-readable sont des vues de découverte et ne remplacent pas les fichiers canoniques versionnés.</p></div><div className="openDataPanel"><article><h3>Dépôt canonique</h3><p>Documents, propositions atomiques, registres, méthodologie, historique Git et licences.</p><a href="https://github.com/TFourniax/programmes-politiques-france-2027" target="_blank" rel="noreferrer">Ouvrir le dépôt ↗</a></article><article><h3>Manifest public JSON</h3><p>Point d’entrée léger pour découvrir l’instantané, les candidats, les thèmes et les URLs publiques.</p><a href="/api/open-data">Ouvrir le manifest →</a></article><article><h3>Accès agents / LLM</h3><p>Fichier de découverte concis indiquant les routes utiles, les limites de preuve et la source canonique.</p><a href="/llms.txt">Lire llms.txt →</a></article></div></section>
