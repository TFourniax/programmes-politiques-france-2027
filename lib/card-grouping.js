...
export function completeSentenceExcerpt(value, target = 680) {
  const text = stripLeadingFragment(value);
...
function firstSourceOrder(card) {
  return card.sourceNumbers?.length ? Math.min(...card.sourceNumbers) : 999;
}

export function groupPoliticalCards(answer, evidence = []) {
  if (!answer || !Array.isArray(answer.cards)) return answer;
  if (!answer.cards.length) return { ...answer, cards: [] };
  ...
  const cards = [...candidateCards.values(), ...nonPartyCards]
    .map((card) => {
      ...
      return { ...clean, __order: __order ?? firstSourceOrder(clean) };
    })
    .sort((a, b) => a.__order - b.__order || firstSourceOrder(a) - firstSourceOrder(b))
    .slice(0, 10)
