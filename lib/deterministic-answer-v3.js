...
function sectionHeading(sentence, item) {
  ...
  return Boolean(sectionKey && sentenceKey === sectionKey);
}
...
function cleanSubtitle(baseSubtitle, titles) {
  ...
  if (/^(attribution|limite(?: d'attribution)?)$/i.test(String(baseSubtitle || "").trim())) return "Corpus documenté";
...
function modeSummary(mode, groupCount, evidenceCount) {
  ...
  if (mode === "comparison") {
    return `Le corpus permet de comparer ${groupCount} ${entityWord} à partir de ${evidenceCount} ${evidenceWord} directement documenté${evidenceCount === 1 ? "" : "s"}.`;
  }
  return `Le corpus contient ${evidenceCount} ${evidenceWord} suffisamment pertinent${evidenceCount === 1 ? "" : "s"} pour répondre. La synthèse reprend uniquement des formulations présentes dans les données versionnées ou les titres canoniques des propositions.`;
}
...
export function composeDeterministicAnswer(question, evidence, { mode = "overview", candidates = [], requestedEntities = [], candidateTargeted = false } = {}) {
  if (mode === "candidates") {
    ...
    return {
      ...base,
      note: `${base.note || ""} Réponse entièrement déterministe, sans génération par LLM.`.trim()
    };
  }
  ...
  return {
    ...
    note: "Réponse entièrement déterministe et extractive : aucune information extérieure au corpus et aucune reformulation générative de position politique n’est ajoutée.",
    sections,
