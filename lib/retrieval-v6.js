...
function residualQualifiers(question) {
...
function unmatchedReturnedQualifiers(question, results) {
  const residual = residualQualifiers(question);
  if (!residual.length) return [];
  const evidence = results.map(evidenceTerms);
...
function enforceReturnedQualifierScope(question, result) {
  if (!result.results?.length) return result;
  const unmatched = unmatchedReturnedQualifiers(question, result.results);
...
export function retrieveDeterministic(question, options = {}) {
  ...
  if (first.debug?.reason !== "unknown_qualifier") return enforceReturnedQualifierScope(question, first);
  const retried = retryWithoutHarmlessQualifiers(question, first.debug, options) || first;
  return enforceReturnedQualifierScope(question, retried);
}
