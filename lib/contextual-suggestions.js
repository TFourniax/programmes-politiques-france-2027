...
function candidatePool(profile) {
  ...
  // Niveau 1 — prolongement immédiat : même thème que la question courante.
  // On privilégie d'abord les acteurs de la réponse, puis les autres acteurs documentés.
  ...
  // Niveau 3 — continuité de session : appliquer un thème récent à un acteur de la réponse courante.
  ...
  // Niveau 4 — approfondissement de l'acteur : autre sujet réellement documenté pour un acteur apparu dans la réponse.
  // Ce niveau ne sert que si les niveaux précédents ne suffisent pas à remplir les suggestions.
  for (const entityId of profile.answerEntities) {
...
export function buildContextualSuggestions(question, evidence = [], history = [], { limit = 3 } = {}) {
  ...
  for (const candidate of pool) {
    ...
    // Deux suggestions maximum sur le même acteur, mais un même thème peut occuper les trois
    // places si trois acteurs différents sont documentés : c'est le comportement voulu pour
    // approfondir naturellement la question courante.
