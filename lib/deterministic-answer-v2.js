...
function proposalTitle(rows) {
  return rows.find((row) => row.item?.citation?.kind === "proposal")?.item?.citation?.title || "";
}
...
export function composeDeterministicAnswer(question, evidence, { mode = "overview", candidates = [] } = {}) {
  ...
  for (const [entityId, rows] of groups) {
    ...
    const titleFallback = proposalTitle(rows);
    ...
    cards.push({
      ...
      sourceNumbers: sourceNumbers.length ? sourceNumbers : [rows.findIndex((row) => row.item?.citation?.kind === "proposal") + 1]
    });
  }
