...
function genericMetaSentence(value = "") {
  return /^(proposition explicitement documentee|attribution|limite|la source|ce document|le document)/.test(normalize(value));
}

function proposalRow(rows) {
  return rows.find((row) => row.item?.citation?.kind === "proposal") || null;
}
...
export function composeDeterministicAnswer(question, evidence, { mode = "overview", candidates = [] } = {}) {
  ...
  for (const [entityId, rows] of groups) {
    ...
    const proposal = proposalRow(rows);
    const titleFallback = proposal?.item?.citation?.title || "";
    if (!extracted.length && !titleFallback) continue;
    ...
    const summary = (!first || genericMetaSentence(first)) && titleFallback ? titleFallback : first;
    cards.push({
      ...baseCard,
      summary,
      bullets: extracted.filter((row) => row.sentence !== summary).slice(0, 4).map((row) => row.sentence),
      sourceNumbers: sourceNumbers.length ? sourceNumbers : proposal ? [proposal.sourceNumber] : []
    });
  }
