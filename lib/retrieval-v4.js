...
const GENERIC = new Set([
  "actuel","actuelle","actuels","actuelles","actuellement","alors","annee","annees","autre","autres","avoir","candidat","candidats","candidature","candidatures",
  "comment","compare","comparaison","concerne","concernent","connaitre","corpus","developpe","developper","developpement","difference","dire","dis",
  "documente","documentee","documentes","documentees","donne","donner","duree","engagement","engagements","est","etre","exemple","exemples","explique",
  "faire","fais","fortement","france","francais","francaise","info","infos","information","informations","long","longue","maniere","mesure","mesures",
  "nouveau","nouveaux","nouvelle","nouvelles","officiel","officiels","parle","parler","peux","peut","politique","politiques","position","positions",
  "programme","programmer","programmes","programmation","projet","projets","propos","propose","proposent","proposition","propositions","prevoit","public",
  "publique","publics","publiques","plusieurs","savoir","source","sources","sujet","sujets","systeme","systemes","temps","type","types","veut","veulent",
  "vision","voudrais"
]);
...
function conceptCoveredTerms(concepts) {
  ...
  for (const concept of concepts) {
    for (const term of tokens([concept.matchedAlias, concept.label, ...(concept.retrieval_terms || [])].filter(Boolean).join(" "))) covered.add(term);
  }
  return covered;
}
...
export function analyzeQuery(question) {
  ...
  const coveredByConcepts = conceptCoveredTerms(concepts);
  const residualSubjectTokens = subjectTokens.filter((term) => !numberSet.has(term) && !coveredByConcepts.has(term));
  return {
    allTokens,
    subjectTokens,
    residualSubjectTokens,
    concepts,
    requestedEntities,
    numbers,
...
function residualCompatible(row, analysis) {
  if (!analysis.residualSubjectTokens.length) return true;
  const hits = analysis.residualSubjectTokens.filter((term) => row.direct.has(term));
  return hits.length >= directMinimum(analysis.residualSubjectTokens.length);
}
function relevant(chunk, row, analysis, minScore) {
  ...
  if (!residualCompatible(row, analysis)) return false;
...
export function retrieveDeterministic(question, { limit = 10, minScore = 2.2 } = {}) {
  ...
  if (analysis.intent.unsupported) {
    return { results: [], debug: { answerable: false, reason: `unsupported_${analysis.intent.unsupportedReason}`, concepts: analysis.concepts, requestedEntities: analysis.requestedEntities, subjectTokens: analysis.subjectTokens, residualSubjectTokens: analysis.residualSubjectTokens, numbers: analysis.numbers, intent: analysis.intent, candidates: PUBLIC_CHUNKS.length, matched: 0, coherent: 0 } };
  }
  ...
  return {
    ...
    debug: {
      ...
      residualSubjectTokens: analysis.residualSubjectTokens,
