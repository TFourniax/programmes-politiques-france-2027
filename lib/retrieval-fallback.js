...
// The retrieval fallback is deliberately isolated from the legacy generative-chat
// configuration. Old LLM_API_URL / LLM_MODEL values must never silently redirect
// this tiny classifier to an incompatible provider or retired model.
const API_URL = process.env.LLM_FALLBACK_API_URL || "https://api.openai.com/v1/chat/completions";
const MODEL = process.env.LLM_FALLBACK_MODEL || "gpt-5-nano";
...
const PRIMARY_COMPLETION_TOKENS = 700;
const RETRY_COMPLETION_TOKENS = 1200;
...
function key() {
  // Prefer a key explicitly dedicated to this fallback. OPENAI_API_KEY is the
  // canonical generic alternative. LLM_API_KEY remains a compatibility fallback
  // so existing Netlify environments keep working when that key is already OpenAI.
  const value = process.env.LLM_FALLBACK_API_KEY || process.env.OPENAI_API_KEY || process.env.LLM_API_KEY || "";
...
export function shouldAttemptRetrievalFallback(debug = {}) {
  if (!ENABLED || !key() || circuitOpen()) return false;
...
async function requestInterpretation(apiKey, system, userContent, maxCompletionTokens) {
  ...
  try {
    ...
    return {
      ...
      refusal: String(choice?.message?.refusal || "").trim(),
...
Tu peux utiliser ta compréhension linguistique générale pour reconnaître des synonymes, paraphrases ou termes techniques, mais tu n'utilises aucune connaissance factuelle extérieure sur les partis, candidats ou programmes et tu ne déduis aucune position politique.
N'invente aucun identifiant et n'ajoute aucun nombre absent de la question.
...
Le serveur n'acceptera que confidence=high, vérifiera chaque mapping, puis revalidera la requête obtenue avec son moteur déterministe.`;
  ...
  const userContent = `QUESTION ACTUELLE:\n${String(question).slice(0, 1200)}\n\nQUESTIONS UTILISATEUR RÉCENTES (contexte seulement):\n${conversation(history) || "Aucune"}\n\n${catalogue()}`;

  try {
    let completion = await requestInterpretation(apiKey, system, userContent, PRIMARY_COMPLETION_TOKENS);
    if (completion.refusal) {
      markSuccess();
      return { attempted: true, interpretation: null, query: "", model: MODEL, error: "fallback_refusal" };
    }

    // GPT-5 family models can spend part of max_completion_tokens on reasoning.
    // A single bounded retry prevents a reasoning-only/empty completion from
    // silently disabling the semantic fallback while keeping cost and latency capped.
    if (!completion.content) {
      completion = await requestInterpretation(apiKey, system, userContent, RETRY_COMPLETION_TOKENS);
      if (completion.refusal) {
        markSuccess();
        return { attempted: true, interpretation: null, query: "", model: MODEL, error: "fallback_refusal" };
      }
    ...
    if (!completion.content) throw new Error(`fallback_empty_response${completion.finishReason ? `_${completion.finishReason}` : ""}`);
    const parsed = JSON.parse(completion.content);
    const interpretation = sanitizeRetrievalInterpretation(parsed, question);
    // A low-confidence/unsupported interpretation means the provider behaved correctly and cautiously.
    // Only transport/protocol failures should count toward the circuit breaker.
    markSuccess();
    return {
      attempted: true,
      interpretation,
