...
const GENERIC = new Set(["alors","avoir","candidat","candidats","candidature","candidatures","compare","comparaison","connaitre","corpus","difference","dire","dis","donne","donner","engagement","engagements","est","etre","exemple","exemples","explique","faire","fais","info","infos","information","informations","mesure","mesures","officiel","officiels","parle","parler","peux","peut","position","positions","programme","programmes","projet","projets","propos","propose","proposent","proposition","propositions","prevoit","savoir","source","sources","sujet","sujets","type","types","veut","veulent","vision","voudrais"]);
...
function detectConcepts(question) {
  const normalized = normalize(question);
  const words = new Set(normalized.split(/\s+/));
  return ontology.concepts.filter((concept) => concept.aliases.some((raw) => {
    const alias = normalize(raw);
    return alias.includes(" ") ? phrase(normalized, alias) : words.has(alias);
  }));
}
...
export function analyzeQuery(question) {
  ...
  return {
    allTokens,
    subjectTokens:allTokens.filter((term) => !GENERIC.has(term)),
    concepts:detectConcepts(question),
    requestedEntities:detectEntities(question),
...
function score(chunk,analysis) {
  ...
  for (const concept of analysis.concepts) { const hit=conceptMatch(concept,data); if(!hit.matched)continue; concepts.push({id:concept.id,label:concept.label,terms:hit.terms}); value+=5.5+Math.min(7,hit.score*0.8); }
...
function relevant(chunk,row,analysis,minScore){
  ...
  if(analysis.concepts.length)return row.concepts.length>0;
...
export function retrieveDeterministic(question,{limit=10,minScore=2.2}={}){
  ...
  const selected=diversify(ranked,limit,analysis.intent.compare);
  return{
    ...
    debug:{answerable:selected.length>0,reason:selected.length?"hybrid_evidence":"insufficient_relevance",concepts:analysis.concepts.map(({id,label})=>({id,label})),requestedEntities:analysis.requestedEntities,subjectTokens:analysis.subjectTokens,numbers:analysis.numbers,intent:analysis.intent,candidates:searchIndex.chunks.length,matched:ranked.length}
  };
}
