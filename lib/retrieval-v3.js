...
function relevant(chunk,row,analysis,minScore){
  ...
  if(analysis.concepts.length){
    ...
    if(analysis.subjectTokens.length&&!row.direct.size)return false;
    return true;
  }
...
export function retrieveDeterministic(question,{limit=10,minScore=2.2}={}){
  ...
  return{
    ...
    debug:{answerable:selected.length>0,reason:selected.length?"hybrid_evidence":"insufficient_relevance",concepts:analysis.concepts.map(({id,label})=>({id,label})),requestedEntities:analysis.requestedEntities,subjectTokens:analysis.subjectTokens,numbers:analysis.numbers,intent:analysis.intent,candidates:searchIndex.chunks.length,matched:ranked.length,coherent:coherent.length}
  };
}
