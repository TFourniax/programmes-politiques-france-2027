...
export function classifyQuestion(question) {
  const q = norm(question);
  const policyIntent = /programme|proposition|mesure|propose|prevoit|engagement|retraite|fiscal|impot|immigration|europe|travail|smic|ecologie|sante|education|securite|logement|energie/.test(q);
  const comparisonIntent = /compare|comparaison|difference|oppos|versus| vs /.test(` ${q} `);
  if (comparisonIntent) return "comparison";

  const candidateStatusIntent = /(?:candidat|candidature).{0,45}(?:declare|officiel|potentiel|probable|possible|primaire|investi|designe|retire|renonce|condition|statut|en course)|(?:declare|officiel|potentiel|probable|possible|primaire|investi|designe|retire|renonce|condition|statut|en course).{0,45}(?:candidat|candidature)/.test(q);
  const plainCandidateList = /(?:qui|quels|quelles|liste|combien).{0,35}(?:candidat|candidature)/.test(q) && !policyIntent;
  if (candidateStatusIntent || plainCandidateList) return "candidates";
  if (policyIntent) return "measures";
  return "overview";
}
