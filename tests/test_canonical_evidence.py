...
def make_proposal(root: Path, proposal_id="claim-1", entity_id="alice", topic="retraites") -> Path:
    ...
    write_md(path, {
        ...
        "title": "Fixer un âge légal documenté",
        "entity_id": entity_id,
        "topic": topic,
        "certainty": "explicit",
    ...
    }, "# Fixer un âge légal documenté\n\nLa candidate propose une mesure explicite et vérifiée sur les retraites.")
    return path


def test_duplicate_target_must_keep_actor_and_topic(tmp_path):
    make_proposal(tmp_path)
    records = proposal_records(tmp_path)
    assert valid_target({"actor_id": "alice", "topic": "retraites"}, "claim-1", records) == "claim-1"
    assert valid_target({"actor_id": "bob", "topic": "retraites"}, "claim-1", records) is None
    assert valid_target({"actor_id": "alice", "topic": "fiscalite-redistribution"}, "claim-1", records) is None
...
def test_resolve_target_can_use_exact_persisted_fingerprint(tmp_path):
    ...
    claim = {
        "actor_id": "alice",
        "topic": "retraites",
        "statement": "La candidate propose de fixer un âge légal documenté.",
    }
...
def test_merge_provenance_is_idempotent_and_never_creates_a_second_claim(tmp_path):
    ...
    claim = {"actor_id": "alice", "topic": "retraites"}
