...
export default function KnowledgeQuiz({ onExplore }) {
  ...
  return <section className="panel explorerPanel quizExplorer">
    ...
    {quiz && <div className="quizBody">
      ...
      {!done && question && <article className="quizCard">
        <div className="quizCounter">Question {index + 1} / {questions.length}</div>
        ...
        <div className="quizOptions">{question.options.map((option, optionIndex) => {
          ...
          return <button key={option} disabled={answered} className={`${isCorrect ? "correct" : ""} ${isWrong ? "wrong" : ""}`} onClick={() => setAnswers((current) => ({...current, [question.id]: optionIndex}))}><span>{String.fromCharCode(65 + optionIndex)}</span>{option}</button>;
        })}</div>
        ...
        <div className="quizNav"><button disabled={index === 0} onClick={() => setIndex((value) => Math.max(0, value - 1))}>← Précédente</button>{answered && index < questions.length - 1 && <button className="primaryAction" onClick={() => setIndex((value) => Math.min(questions.length - 1, value + 1))}>Question suivante →</button>}</div>
      </article>}

      {done && <div className="quizResult">
        ...
        <div className="quizResultActions"><button onClick={() => { setAnswers({}); setIndex(0); }}>Recommencer</button>{quiz.topic && <button onClick={() => onExplore?.(`Explique-moi les principales positions actuellement documentées sur ${quiz.topic.label}, avec leurs sources et les informations encore manquantes.`)}>Approfondir ce thème dans le chat ↗</button>}</div>
        <div className="quizReview">{questions.map((item, itemIndex) => <button key={item.id} onClick={() => setIndex(itemIndex)} className={answers[item.id] === item.correctIndex ? "correct" : "wrong"}>{itemIndex + 1}<span>{answers[item.id] === item.correctIndex ? "✓" : "×"}</span></button>)}</div>
      </div>}
