...
assert.ok(deterministic.hitAt5 >= 0.83, `deterministic hit@5 too low: ${JSON.stringify(deterministic)}`);
assert.ok(deterministic.mrr >= 0.62, `deterministic MRR too low: ${JSON.stringify(deterministic)}`);
...
assert.ok(deterministic.mrr >= legacy.mrr - 0.03, `deterministic retrieval regressed MRR too far vs legacy: legacy=${legacy.mrr}, v2=${deterministic.mrr}`);
...
const route = fs.readFileSync(path.resolve('app/api/chat/route.js'), 'utf8');
