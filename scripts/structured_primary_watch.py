...
def same_host(a: str, b: str) -> bool:
    return urlsplit(a).netloc.lower().removeprefix("www.") == urlsplit(b).netloc.lower().removeprefix("www.")


def request_json(session: requests.Session, url: str, *, params: dict[str, Any] | None = None, attempts: int = 2) -> requests.Response:
    errors: list[str] = []
    profiles = [
        {},
        {
    ...
    for attempt in range(max(1, attempts)):
        for headers in profiles:
            try:
                response = session.get(url, params=params, headers=headers or None, timeout=30, allow_redirects=True)
            ...
            if response.status_code == 200:
                ...
                if "json" not in (response.headers.get("content-type") or "").lower():
                    raise ValueError(f"structured REST returned non-JSON content: {response.headers.get('content-type')}")
                return response
...
def direct_posts_rows(session: requests.Session, api_base: str, source: dict[str, Any]) -> tuple[list[dict[str, Any]], str, int]:
    params = {
        "search": str(source.get("search") or "Chapitre"),
        "per_page": 100,
        "_fields": "id,date,modified,slug,link,title,content,status",
    }
    endpoint = f"{api_base}/wp-json/wp/v2/posts"
    response = request_json(session, endpoint, params=params, attempts=1)
    ...
    try:
        ...
        total_pages = 1
    if total_pages != 1:
        raise ValueError(f"direct WordPress query unexpectedly paginated ({total_pages})")
    return payload, f"{endpoint}?{urlencode(params)}", 1


def search_then_detail_rows(session: requests.Session, api_base: str, source: dict[str, Any]) -> tuple[list[dict[str, Any]], str, int]:
    search_params = {
        "search": str(source.get("search") or "Chapitre"),
        "per_page": 100,
        "_fields": "id,title,url,type,subtype",
    }
    search_endpoint = f"{api_base}/wp-json/wp/v2/search"
    response = request_json(session, search_endpoint, params=search_params, attempts=2)
    found = response.json()
    if not isinstance(found, list):
        raise ValueError("WordPress search response is not a list")
    pattern = re.compile(str(source.get("link_pattern") or r"/programme2025/livre/chapitre\d+/?$"), flags=re.I)
    candidates = []
    for item in found:
        public_url = canonicalize_url(str(item.get("url") or ""))
        title = html.unescape(str(item.get("title") or "")).strip()
        if not public_url or not pattern.search(urlsplit(public_url).path):
            continue
        if not re.match(r"^Chapitre\s+\d+\s*:", title, flags=re.I):
            continue
        candidates.append({"id": int(item["id"]), "subtype": str(item.get("subtype") or "post")})
    if not candidates:
        raise ValueError("WordPress search returned no programme chapter candidates")

    rows = []
    errors = []
    for item in sorted(candidates, key=lambda row: row["id"]):
        rest_base = item["subtype"] if re.fullmatch(r"[a-z0-9_-]+", item["subtype"]) else "post"
        rest_base = "posts" if rest_base == "post" else rest_base
        url = f"{api_base}/wp-json/wp/v2/{rest_base}/{item['id']}"
        try:
            detail = request_json(session, url, params={"_fields": "id,date,modified,slug,link,title,content,status"}, attempts=2)
            payload = detail.json()
    ...
    if errors:
        raise ValueError("incomplete WordPress detail retrieval: " + " | ".join(errors[:5]))
    return rows, f"{search_endpoint}?{urlencode(search_params)} -> individual details", 1


def fetch_wordpress_rows(session: requests.Session, api_base: str, source: dict[str, Any]) -> tuple[list[dict[str, Any]], str, int, str]:
    direct_error = None
    try:
        rows, endpoint, pages = direct_posts_rows(session, api_base, source)
        return rows, endpoint, pages, "direct_posts_with_content"
    except (requests.RequestException, ValueError, TypeError, json.JSONDecodeError) as exc:
        direct_error = f"{type(exc).__name__}: {exc}"
    try:
        rows, endpoint, pages = search_then_detail_rows(session, api_base, source)
        return rows, endpoint, pages, "search_then_individual_details"
    except (requests.RequestException, ValueError, TypeError, json.JSONDecodeError) as exc:
        raise ValueError(f"all official WordPress transports failed; direct=[{direct_error}]; search/detail=[{type(exc).__name__}: {exc}]") from exc
...
def _link_allowed(link: str, source: dict[str, Any]) -> bool:
    ...
    if pattern and not re.search(pattern, link, flags=re.I):
        return False
    return not any(str(fragment).lower() in link.lower() for fragment in source.get("exclude_link_fragments", []) or [])
...
def wordpress_chapters(session: requests.Session, source: dict[str, Any]) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    rows, endpoint, pages, transport = fetch_wordpress_rows(session, api_base, source)
    by_number = {}
    rejected = 0
    out_of_scope = 0
    for row in rows:
    ...
    health = {
        "id": str(source["id"]), "owner": str(source["owner"]), "kind": "wordpress_programme_chapters",
        "status": 200 if complete else 206, "checked_at": iso_now(), "api_endpoint": endpoint,
        "transport": transport, "minimum_expected_items": minimum_expected, "expected_items": expected_items,
        "item_count": len(chapters), "full_content_items": sum(1 for item in chapters if item["text_chars"] >= min_chars),
        "rejected_items": rejected, "out_of_scope_items": out_of_scope,
        "coverage_urls": [canonicalize_url(str(url)) for url in source.get("coverage_urls", [])],
        "chapter_numbers": numbers, "highest_chapter_number": highest, "contiguous": contiguous,
        "wordpress_pages_fetched": pages, "minimum_content_chars": min([item["text_chars"] for item in chapters], default=0),
        "maximum_content_chars": max([item["text_chars"] for item in chapters], default=0), "complete": complete,
    }
...
def collect_source(session: requests.Session, source: dict[str, Any], previous: dict[str, Any]) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    for item in chapters:
        ...
        if not old or changed:
            events.append({
                "event_type": "official_source_changed" if changed else "official_new_url", "observed_at": iso_now(),
                "owner": str(source["owner"]), "priority": "high", "published_at": item.get("date"),
                "source_tier": "tier_1_primary_official", "title": item["title"], "url": item["link"],
                "fetch_url": item["fetch_url"], "sha256": item["sha256"], "verification_state": "needs_review",
                "provenance": "official_wordpress_rest_full_primary_content", "structured_source_id": str(source["id"]),
...
def write_report(events: list[dict[str, Any]], health_rows: list[dict[str, Any]], warnings: list[str]) -> None:
    ...
    lines = [f"# Sources primaires structurées — {day}", "", f"Généré automatiquement à `{iso_now()}`.", ""]
    for row in health_rows:
        lines += [f"## {row.get('owner')} · {row.get('id')}", "", f"- transport REST : {row.get('transport')} ;", f"- chapitres : {row.get('item_count')} (minimum connu : {row.get('minimum_expected_items')}) ;", f"- séquence continue : {'oui' if row.get('contiguous') else 'non'} ;", f"- objets à contenu complet : {row.get('full_content_items')} ;", f"- état : {'complet' if row.get('complete') else 'incomplet'} ;", f"- nouveaux/changés : {row.get('event_count', 0)}.", ""]
    if warnings:
        lines += ["## Avertissements", "", *[f"- {item}" for item in warnings], ""]
...
def main() -> None:
    ...
    all_events, health_rows, warnings = [], [], []
    ...
    for source in sources:
        ...
        try:
            ...
            current[source_id] = {**dict(previous.get(source_id) or {}), "id": source_id, "owner": str(source.get("owner") or ""), "kind": str(source.get("kind") or ""), "status": 0, "checked_at": iso_now(), "complete": False, "error": warning, "coverage_urls": [canonicalize_url(str(url)) for url in source.get("coverage_urls", [])]}
    ...
    for row in health_rows:
        print(f"  {row['id']}: transport={row['transport']}, {row['item_count']} chapter(s), baseline={row['minimum_expected_items']}, complete={row['complete']}")
