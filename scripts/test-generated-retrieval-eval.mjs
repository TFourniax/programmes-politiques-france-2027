...
for (const proposal of proposals) {
  ...
  for (const [variant, question] of variants.entries()) {
    cases.push({ id: `${proposal.path}#${variant + 1}`, question, expectedPath: proposal.path });
  }
}
...
assert.ok(metrics.hit5 >= 0.875, `generated evaluation hit@5 too low: ${metrics.hit5.toFixed(3)}; failures=${JSON.stringify(failures.slice(0, 20))}`);
...
for (const [variant, row] of Object.entries(metrics.variants)) {
  assert.ok(row.hit5 >= 0.75, `generated evaluation variant ${variant} hit@5 too low: ${row.hit5.toFixed(3)}`);
}

console.log('Generated retrieval evaluation OK', {
